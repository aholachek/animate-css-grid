...
animateMutations = (container, { duration = 300, stagger }) => {
  ...
  const recordPositions = elements => {
    ;[...elements].forEach(el => {
      ...
      data.forEach(d => (el.dataset[`cached${d[0].toUpperCase()}${d.slice(1)}`] = rect[d]))
    })
  }
  ...
  const mutationCallback = mutationsList => {
    ...
    mutationsList.filter(m =>
      m.type === 'childList'
    ).length && recordPositions(container.children)

    // then, check if the grid or grid items have had classes added or removed
    const relevantMutationHappened = mutationsList.filter(m => m.attributeName === "class").length

    if (!relevantMutationHappened) return

      ;[...container.children].forEach((el, i, children) => {

        const { top, left, width, height } = el.getBoundingClientRect()
        const { cachedTop, cachedLeft, cachedWidth, cachedHeight } = el.dataset
        if (top === cachedTop && left === cachedLeft && width === cachedWidth && height === cachedHeight) return
        ...
        if ([...el.children].length > 1) throw new Error('Make sure every grid item has a single container element surrounding its children')
